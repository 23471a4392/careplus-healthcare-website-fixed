...
import { Plus, Lock, Mail, Eye, EyeOff, ArrowRight, ShieldCheck, User, Phone, AlertCircle, CheckCircle } from 'lucide-react';
...
export const LoginPage: React.FC<LoginPageProps> = ({ portalKey }) => {
  ...
  // Dynamic state (starts empty, user enters their own credentials)
  ...
  const handleLoginSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!success) {
        setErrorMessage('Invalid credentials or account not authorized for this portal.');
      }
    } catch (err: any) {
  ...
  return (
    <div className="min-h-screen bg-[#f3f8f7] text-[#132e2b] flex flex-col justify-between">
          ...
          {activeTab === 'signin' && (
            <form onSubmit={handleLoginSubmit} className="space-y-4">
              ...
              <button
                ...
                className="w-full py-2.5 bg-[#0c756e] hover:bg-[#095e58] text-white rounded-xl text-xs font-bold flex items-center justify-center gap-2 shadow-sm transition disabled:opacity-50 mt-2"
              >
                {isSubmitting ? (
                  <span>Signing In...</span>
                ) : (
                  <>
                    <span>Sign In to {portalInfo.title}</span>
                    <ArrowRight className="w-3.5 h-3.5" />
                  </>
                )}
              </button>
      ...
      <footer className="py-4 text-center text-xs text-[#6b9690] border-t border-slate-100 bg-white">
        CarePlus Hospital Information Management System · Dynamic Multi-Role Authentication
      </footer>
    </div>
  );
};
